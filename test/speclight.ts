...
export const given: BddTemplateTagFunction = function (strings: TemplateStringsArray, ...args) {
	return (step: Function) => {
		bddIt('Given', strings, args, step);
	};
};

export const when: BddTemplateTagFunction = function (strings: TemplateStringsArray, ...args) {
	return (step: Function) => {
		bddIt('When', strings, args, step);
	};
};

export const then: BddTemplateTagFunction = function (strings: TemplateStringsArray, ...args) {
	return (step: Function) => {
		bddIt('Then', strings, args, step);
	};
};

export const and: BddTemplateTagFunction = function (strings: TemplateStringsArray, ...args) {
	return (step: Function) => {
...
function formatArg(args: object[], i: number) {
	...
	switch (typeof a) {
		...
		case 'object': return JSON.stringify(a, undefined, 2);
	}
